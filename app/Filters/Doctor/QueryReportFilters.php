...
class QueryReportFilters extends Filters
{
    ...
    protected $filters = ['from', 'to', 'search', 'type', 'sortBy', 'report', 'status', 'report_type', 'reportable_id'];
    ...
    public function reportable_id($value)
    {

        $this->builder->where(function ($q) use ($value) {
            $q->whereIn('reportable_id', function ($q) use ($value) {
                $q->select('id')->from('appointments')
                    ->where('bookable_id', $value)
                    ->where('bookable_type', 'App\Models\User');
            })
                ->orWhere('reportable_id', $value);
        });
    }
