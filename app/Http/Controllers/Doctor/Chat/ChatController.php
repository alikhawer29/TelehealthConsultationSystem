...
namespace App\Http\Controllers\Doctor\Chat;
    ...
    public function index(ChatFilters $filter)
    {
        try {
            ...
            $data->getCollection()->transform(function ($chat) use ($user, $admin) {
                ...
                if ($chat->chat_type == 'admin_doctor') {
                ...
                return [
                    ...
                    'is_booking_open' => $chat->type === 'general'
                        ? true
                        : ($chat->type === 'session'
                            && $chat->chat_type === 'user_doctor'
                            && $chat->appointment?->appointment_status === 'inprogress'),

                    // 'is_booking_open' => $chat->chat_type === 'admin_doctor'
                    //     ? true
                    //     : ($chat->chat_type === 'user_doctor' && $chat->appointment?->appointment_status === 'inprogress' && $chat->type === 'session'),
        ...
        try {
            ...
            $data->getCollection()->transform(function ($chat) use ($admin) {
                return [
                    ...
                    'chat_type' => $chat->chat_type,
                    'appointment_id' => $chat->appointment_id,
                    // 'is_booking_open' => $chat->appointment?->appointment_status === 'inprogress',
                    'created_at' => $chat->created_at,
                    'status' => $chat->status,
                    'type' => $chat->type,
