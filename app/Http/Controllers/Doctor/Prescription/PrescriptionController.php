...
class PrescriptionController extends Controller
{
    ...
    public function index(PrescriptionFilters $filter): JsonResponse
    {
        try {
            $filter->extendRequest([
                ...
                'patient' => true,
                // 'doctor_patients_only' => request()->user()->id,
            ]);
    ...
    private function sendPrescriptionNotifications($prescription, string $action): void
    {
        try {
            ...
            $designation = $doctor->service_provider->professional ?? ucfirst($doctor->role ?? 'Doctor');
            ...
            $adminPayload = [
                ...
                'body'  => "Dr. {$doctorName} has been uploaded a prescription for patient {$patientName}.",
                'data'  => [
                    'id'   => $prescription->id,
                    'type' => 'prescription',
                ],
