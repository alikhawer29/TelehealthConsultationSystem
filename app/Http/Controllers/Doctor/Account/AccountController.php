...
class AccountController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $data = $this->account
                ->getProfile(relations: ['file:id,path,fileable_id,fileable_type', 'education', 'license.file', 'sessionType']);
            if ($data->status == 1) {;
                $data = api_successWithData('Doctor Details',  $data);
    ...
    public function update(DoctorUpdateAccountRequest $request)
    {
        try {
            $data = $this->account->updateDoctor($request->validated());
            $data = api_successWithData('Doctor Updated', $data);
